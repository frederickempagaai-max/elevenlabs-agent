...
export default function ConversationView({ presets }: { presets: Preset[] }) {
  ...
  const nextAvatar = useCallback(() => {
    if (!hasPresets || status === "connecting" || status === "connected") return;
    setSelectedIndex((prev) => (prev + 1) % presets.length);
  }, [hasPresets, presets.length, status]);
    ...
    <>
      <div
        ...
        style={{
          ...
          pointerEvents:
            status === "connecting" || status === "connected" ? "none" : "auto",
        }}
      >
        <a
      ...
      <div
        ...
        style={{
          opacity: status === "connecting" || status === "connected" ? 0 : 1,
          maxHeight:
            status === "connecting" || status === "connected" ? 0 : "200px",
          pointerEvents:
            status === "connecting" || status === "connected" ? "none" : "auto",
        }}
      >
      ...
      <div className="w-full flex flex-col items-center gap-2 sm:gap-4">
        <div className="w-full flex justify-center items-center gap-3 sm:gap-4 flex-shrink">
          ...
          <div className="relative w-full max-w-lg lg:max-w-xl">
            <div className="relative w-full aspect-[720/480] rounded-2xl sm:rounded-[32px] overflow-hidden bg-black">
              <video
                id="avatar-video"
                autoPlay
                playsInline
              ...
              {status === "idle" && !currentPreset && (
                <div
                  className="absolute inset-0 flex items-center justify-center cursor-pointer bg-black text-white/80 text-sm"
                  onClick={start}
                >
        ...
        {presets.length > 1 && (
          <div
            ...
            style={{
              opacity:
                status === "connecting" || status === "connected" ? 0 : 1,
              maxHeight:
            ...
            {presets.map((preset, i) => {
              ...
              return (
                <button
                  ...
                  tabIndex={
                    status === "connecting" || status === "connected" ? -1 : 0
                  }
        ...
        <div className="flex flex-col items-center gap-2 sm:gap-3 w-full max-w-lg lg:max-w-xl px-2">
          {status === "error" && error && (
            <span className="text-xs sm:text-sm text-accent px-4">
              Error: {error}
            </span>
          )}
        </div>
        ...
        {status === "connected" && (
          <div className="w-full max-w-lg lg:max-w-xl h-60 sm:h-64 rounded-2xl sm:rounded-[32px] bg-white overflow-hidden transition-all duration-700 ease-out motion-reduce:transition-none">
            <div
              ref={transcriptRef}
